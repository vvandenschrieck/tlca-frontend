<template>
  <div>
    <div v-if="hasCategories" align="right">
      <v-select v-model="structure" :items="view" item-text="by" item-value="value" dense outlined hide-details height="20" class="d-inline-block text-caption">
        <template #selection="{ item }">
        <v-list-item dense class="pa-0 showby">
          <v-list-item-content class="pa-0"><v-list-item-title>{{ item.by }}</v-list-item-title></v-list-item-content>
        </v-list-item>
      </template>
      </v-select>
    </div>
    <div v-for="category in competencies" :key="category.name">
      ...
      <v-expansion-panels accordion multiple tile flat>
        <v-expansion-panel v-for="{ competency } in category.values" :key="competency.code">
          <v-expansion-panel-header class="px-0 name">
            {{ competency.code }} – {{ competency.name }}
          </v-expansion-panel-header>
          <v-expansion-panel-content>
            <v-card flat>
              <v-card-text class="text--primary font-italic ma-0 pa-0" v-html="description(competency)"></v-card-text>
            </v-card>
          </v-expansion-panel-content>
        </v-expansion-panel>
      </v-expansion-panels>
